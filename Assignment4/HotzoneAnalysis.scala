...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val res = spark.sql("select rectangle, count(point) as point_num from joinResult group by rectangle order by rectangle").persist()
    val finalDF = res.sort("rectangle").coalesce(1)
    return finalDF
  }
